#!/usr/bin/env python3
"""Email the Editor once when new AYIL manuscript numbers appear."""
...
STATE_PATH = Path(".notification-state/yearbook-ids.json")
...
def fetch_register(api_base: str, admin_key: str) -> list[dict]:
    request = Request(
        api_base.rstrip("/") + "/yearbook/admin",
...
def build_message(entries: list[dict], sender: str, recipient: str) -> EmailMessage:
    count = len(entries)
    subject = "[AYIL] " + (f"New submission: {entries[0].get('id', 'number unrecorded')}" if count == 1 else f"{count} new submissions")
    lines = [
...
def main() -> None:
    ...
    recipient = required("AYIL_NOTIFICATION_TO")
